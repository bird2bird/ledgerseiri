...
} = require("../dist/src/imports/dto/amazon-sp-api-orders-api-read-boundary-contract.dto");
...
// Step140-A-FIX1:
// Existing OAuth/LWA/token persistence services are valid preconditions for Orders API.
// This scanner must block newly introduced Orders API read execution, not pre-existing
// token exchange / token persistence infrastructure from Step127-139.
...
function assertNoStep140AImplementationLeak(repoRoot) {
  ...
  assert(routeLeaks.length === 0, `no Step140-A Orders API controller route leak: ${JSON.stringify(routeLeaks)}`);
  assert(serviceLeaks.length === 0, `no Step140-A Orders API service implementation leak: ${JSON.stringify(serviceLeaks)}`);
  assert(httpLeaks.length === 0, `no Step140-A real HTTP/signing/RDT implementation leak: ${JSON.stringify(httpLeaks)}`);
  assert(realOrdersCallLeaks.length === 0, `no Step140-A getOrders/getOrder/getOrderItems leak: ${JSON.stringify(realOrdersCallLeaks)}`);
  assert(writeLeaks.length === 0, `no Step140-A ImportJob/StagingRow/Transaction/Inventory write leak: ${JSON.stringify(writeLeaks)}`);
  assert(schemaLeaks.length === 0, `no Step140-A Prisma schema model leak: ${JSON.stringify(schemaLeaks)}`);
...
async function main() {
  ...
  console.log("========== Step140-A Amazon SP-API Orders API read boundary contract smoke ==========");

  const packageJson = JSON.parse(read(path.resolve(apiRoot, "package.json")));
  assert(
    packageJson.scripts["smoke:amazon-sp-api-orders-api-read-boundary-contract"] ===
      "node scripts/smoke-amazon-sp-api-orders-api-read-boundary-contract.js",
    "apps/api package.json registers Step140-A smoke",
  );

  assert(
    packageJson.scripts["smoke:amazon-sp-api-connection-status-read-model-contract"],
    "Step123-G connection status read-model contract regression script remains registered",
  );
  ...
  const requiredMarkers = [
    "AMAZON_SP_API_ORDERS_API_READ_BOUNDARY_CONTRACT_VERSION",
    "buildAmazonSpApiOrdersApiReadBoundaryContract",
    "assertAmazonSpApiOrdersApiReadBoundaryContract",
    "assertAmazonSpApiConnectionStatusReadModelContract",
    "sourceConnectionStatusReadModel",
    "design-amazon-sp-api-orders-api-read-boundary-contract-only",
    "contractOnly: true",
    "implementationNow: false",
    "realOrdersApiHttpCallNow: false",
    "writesDatabase: false",
    "companyStoreMarketplaceRegionIsolationRequired",
    "getOrdersForbiddenNow",
    "getOrderForbiddenNow",
    "getOrderItemsForbiddenNow",
    "realAmazonHttpForbiddenNow",
    "rawPayloadForbidden",
    "buyerInfoRedactedOnly",
    "shippingAddressRedactedOnly",
    "rawBuyerNameForbidden",
    "rawBuyerEmailForbidden",
    "rawShippingAddressForbidden",
    "normalizedOrderContractOnly",
    "importJobPersistenceDeferred",
    "importStagingRowPersistenceDeferred",
    "transactionCommitDeferred",
    ...
    "sigV4AuthorizationHeaderForbidden",
    "rawNextTokenForbiddenInLogs",
    "rawAmazonPayloadForbidden",
    "crossCompanyReadForbidden",
    "crossStoreReadForbidden",
    "crossMarketplaceReadForbidden",
    "crossRegionReadForbidden",
    "readyForOrdersApiRequestBuilderContract",
    "readyForOrdersApiHttpClientImplementation",
  ];

  const requiredSmokeMarkers = [
    "Step140-A-FIX1",
    "isAllowedExistingAuthOrTokenInfrastructureFile",
    "amazon-sp-api-token-exchange.service.ts",
    "AmazonOrdersApiHttpClient",
    "AmazonSpApiOrdersHttpClient",
  ];

  const smokeSource = read(path.resolve(apiRoot, "scripts/smoke-amazon-sp-api-orders-api-read-boundary-contract.js"));
  for (const marker of requiredSmokeMarkers) {
    assert(smokeSource.includes(marker), `Step140-A smoke scanner marker exists: ${marker}`);
  }

  for (const marker of requiredMarkers) {
    assert(dtoSource.includes(marker), `Step140-A DTO marker exists: ${marker}`);
  }
  ...
  for (const marker of forbiddenSourceMarkers) {
    assert(!dtoSource.includes(marker), `Step140-A DTO does not contain implementation marker: ${marker}`);
  }

  const contract = assertAmazonSpApiOrdersApiReadBoundaryContract(
    buildAmazonSpApiOrdersApiReadBoundaryContract(),
  );

  assert(contract.step === "Step140-A", "contract step is Step140-A");
  ...
  assert(contract.summary.readyForOrdersApiRequestBuilderContract === true, "next contract step is allowed");
  assert(contract.summary.readyForOrdersApiHttpClientImplementation === false, "real HTTP implementation is not allowed yet");

  const implementationGuard = assertNoStep140AImplementationLeak(repoRoot);

  console.log("[SMOKE_OK] Step140-A Amazon SP-API Orders API read boundary contract smoke passed");
  console.log(
    JSON.stringify(
      {
        ok: true,
        ...
        contract: {
          ...
          signedRequestAddedNow: contract.signedRequestAddedNow,
