  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <MetricCard
          label="注文明細"
          value={summary.itemCount.toLocaleString("ja-JP")}
          hint={`staging rows ${summary.totalRows.toLocaleString("ja-JP")}`}
        />
        <MetricCard
          label="SKU"
        ...
        <MetricCard
          ...
          hint="quantityOrdered"
        />
        <MetricCard
          label="商品金額合計"
          value={`¥${Math.round(summary.totalAmount).toLocaleString("ja-JP")}`}
          hint="itemPriceAmount"
        />
        <MetricCard
          label="注文IDなし"
          value={summary.rowsWithoutAmazonOrderId.toLocaleString("ja-JP")}
          hint="要データ確認"
        />
      </div>

      <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-800">
        Step144-A は表示専用です。Transaction作成・InventoryMovement作成・在庫扣減は行いません。
        GetOrderItems failed の注文ヘッダー保存は Step145 で header staging row として追加します。
      </div>
    </section>
  );
}
