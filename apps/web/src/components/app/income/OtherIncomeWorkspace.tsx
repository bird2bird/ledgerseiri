...
// Step109-Z1-H7E-OTHER-INCOME-STATUS-BADGES:
...
function buildOtherIncomeStatusBadges(row: IncomeRow): IncomeStatusBadgeItem[] {
  const memo = String(row.memo || "");
  const account = String(row.account || "");
  const hasScope = memo.includes("[other-income-category:");
  const hasCategory = memo.includes("[other-income-category:");
...
function buildOtherIncomeDenseDashboardPoints(args: {
  ...
  for (const row of rows) {
    const rowMs = parseOtherIncomeDateMs(row);
    if (!rowMs) continue;

    const rowDate = cloneOtherIncomeDate(new Date(rowMs));
    if (Number.isNaN(rowDate.getTime())) continue;

    if (customBounds) {
      const ms = rowDate.getTime();
      if (ms < customStartMs || ms > customEndMs) continue;
...
export function OtherIncomeWorkspace(props: OtherIncomeWorkspaceProps) {
  ...
  const isIncomeImportReturnHighlightedRow = React.useCallback(
    (row: IncomeRow) => {
      ...
      // Step109-Z1-H7D-FIX2C-OTHER-ROW-CONTAINER-HIGHLIGHT:
      // Fallback for rows whose display memo has already stripped internal markers.
      const visibleMemo = String(getOtherIncomeMemo(row) || "");
...
const otherIncomeTrendPoints = React.useMemo(
    () =>
      buildOtherIncomeDenseDashboardPoints({
        rows: otherIncomeDashboardRows,
        range: otherIncomeDashboardRange,
        granularity: otherIncomeTrendGranularity,
        customBounds:
...
const otherIncomeStatusPoints = React.useMemo(
    () =>
      buildOtherIncomeDenseDashboardPoints({
        rows: otherIncomeDashboardRows,
        range: otherIncomeDashboardRange,
        granularity: otherIncomeStatusGranularity,
        customBounds:
          otherIncomeDashboardRange === "custom" ? otherIncomeCustomRangeBounds : null,
      }),
    [
        ...
        <div className="rounded-[30px] border border-slate-200/80 bg-white p-6 shadow-[0_18px_48px_rgba(15,23,42,0.06)]">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <div className="mt-2 text-sm leading-6 text-slate-500">
                期間別の収入合計を比較します。表示範囲は左側の収入推移と連動し、収入がない日も 0 円として含めます。
              </div>
            </div>
                ...
                >
                  ...
                  <div>
                    <div className="text-slate-600">
                      <div>{getOtherIncomeMemo(row) || "-"}</div>
                      {renderIncomeStatusBadges(buildOtherIncomeStatusBadges(row))}
                    </div>
