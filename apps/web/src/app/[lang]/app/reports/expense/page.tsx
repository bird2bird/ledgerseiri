...
export default function ExpenseReportPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        setLoading(true);
        setError("");
        const res = await fetchExpenseReport({ range, storeId });
        if (!alive) return;
        setData(res);
      } catch (e: unknown) {
        if (!alive) return;
  ...
  const vm = useMemo<ReportPageVm>(() => {
    ...
    return {
      reportKey: "expense",
      title: "支出分析",
      description: "支出合計・件数・平均・費目別推移を確認します。",
      range,
      storeId,
 ...
 tone: "warning" },
      ...
      trendColumns: [
        { key: "date", label: "Date" },
        { key: "amount", label: "Expense", align: "right" },
      ...
      trendRows: trend.map((row: any, idx: number) => ({
        ...
        values: {
          ...
          amount: fmtJPY(Number(row.amount ?? row.totalExpense ?? 0)),
