...
function badgeForType(t: Tx['type']) {
  ...
  switch (t) {
    case 'SALE':
      return cls(base, 'border-emerald-200 bg-emerald-50 text-emerald-700');
    case 'FBA_FEE':
      return cls(base, 'border-slate-200 bg-slate-50 text-slate-700');
    case 'AD':
      return cls(base, 'border-indigo-200 bg-indigo-50 text-indigo-700');
    case 'REFUND':
      return cls(base, 'border-rose-200 bg-rose-50 text-rose-700');
    default:
      return cls(base, 'border-amber-200 bg-amber-50 text-amber-700');
  }
}
...
const I18N: Record<Lang, Record<string, string>> = {
  ja: {
    ...
    insights: '経営洞察',
    block4Sub: 'Block 4 · Insights (Trends / Mix)',
    monthIncome: '月収入',
    monthExpense: '月支出',
    ...
    live: 'Live',
    score: 'Score',
    healthMessageTitle: 'Health message',
    healthMessage: '好調です。この状態を維持しつつ、次はキャッシュフローを整えましょう。',
    profitRate: '利益率',
    refundRate: '返金率',
    adsRate: '広告比率',
    // forms
    ...
    // todo/notice
    todo: '待办事项',
    todoSub: 'Block 7 · System tasks',
    todoEmpty: '未処理の重要タスクはありません（この調子です）',
    todoHint: '推奨：1〜2分で確認',
    low: 'Low',
    notice: '消息通知',
    noticeSub: 'Block 8 · System / Product',
    markRead: '全部已读',
    notice1: 'ダッシュボードの見た目を freee 風に刷新しました。',
    notice2: '今月の取引：9 件を集計しました。',
    tips: 'Tips: 費用（FBA/AD/REFUND）は DB にそのまま保存されます（マイナスで入っている前提）。',
  },
  ...
  en: {
    ...
    score: 'Score',
    healthMessageTitle: 'Health message',
    healthMessage: 'Looks good. Keep this pace and improve cashflow next.',
    ...
    action: 'Action',
    delete: 'Delete',
    noTx: 'No transactions in this month.',
    ...
    todo: 'To-do',
    todoSub: 'Block 7 · System tasks',
    todoEmpty: 'No critical tasks pending (nice).',
    todoHint: 'Suggested: review in 1–2 min',
    low: 'Low',

    notice: 'Notifications',
    noticeSub: 'Block 8 · System / Product',
    markRead: 'Mark all read',
    notice1: 'Refreshed dashboard UI in a freee-like style.',
    notice2: 'This month: aggregated 9 transactions.',
    tips: 'Tips: Expense types (FBA/AD/REFUND) are stored as-is in DB (negative amounts).',
  },
...
const currentLang = (lang as Lang) || 'ja';
const [uiLang, setUiLang] = useState<Lang>(currentLang);
...
async function refresh() {
  ...
  const score = dash ? Math.round(Math.min(100, Math.max(0, (dash.sales ? (net / Math.max(1, dash.sales)) : 0) * 100))) : 0;
        ...
        <div className="sticky top-0 z-20 border-b border-black/5 bg-white/80 backdrop-blur">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#2b5cff] text-white text-sm font-bold shadow-sm">
                LS
              </div>
              <div className="leading-tight">
                <div className="text-sm font-semibold tracking-tight">{t("appName")}</div>
                <div className="text-[12px] text-slate-500">{company ? company.name : t("demoCompany")}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden lg:flex items-center">
                <div className="relative">
                  <input
                    className="w-[280px] rounded-full border border-black/10 bg-white/80 px-4 py-2 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2b5cff]/25"
                    placeholder="Search… (⌘K)"
                    onFocus={() => {
                      // hint only; palette is opened by keyboard shortcut
                    }}
                    readOnly
                  />
...
<div className="mx-auto grid max-w-7xl grid-cols-12 gap-4 px-4 py-6">
        ...
        <aside className="col-span-12 lg:col-span-3">
          <div className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <nav className="mt-2 space-y-1 text-sm">
              <a className="block rounded-xl bg-[#eaf1ff] px-3 py-2 font-medium text-[#2b5cff]" href="#">{t('home')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('ledgerList')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('journalList')}</a>
              <div className="my-2 border-t" />
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('purchase')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('payment')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('expense')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('otherIncome')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('otherExpense')}</a>
              <div className="my-2 border-t" />
              <div className="text-[12px] text-slate-500 px-3">{t('financeReport')}</div>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('profitReport')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('cashflow')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('detail')}</a>
              <div className="my-2 border-t" />
              <div className="text-[12px] text-slate-500 px-3">{t('tax')}</div>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('vat')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('incomeTax')}</a>
              <a className="block rounded-xl px-3 py-2 hover:bg-slate-50" href="#">{t('annual')}</a>
            </nav>
          </div>
        </aside>
        ...
        <main className="col-span-12 lg:col-span-6 space-y-4">
          ...
          <section className="rounded-2xl border bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between">
              ...
              <span className="inline-flex items-center rounded-full bg-[#eaf1ff] px-2 py-1 text-[11px] font-medium text-[#2b5cff]">
                {t('live')}
              </span>
            </div>
            ...
            <div className="mt-4 grid grid-cols-12 gap-3">
              <div className="col-span-12 md:col-span-7">
                ...
                <div className="mt-4 grid grid-cols-4 gap-2">
                  <select
                    className="col-span-1 rounded-xl border px-3 py-2 text-sm"
                    value={type}
                ...
                <button
                  className="mt-3 w-full rounded-xl bg-[#2b5cff] px-4 py-2 text-sm font-semibold text-white hover:opacity-95 disabled:opacity-50"
              ...
              <div className="col-span-12 md:col-span-5">
                <div className="rounded-2xl border bg-[#f6f9ff] p-4">
          ...
          <section className="grid grid-cols-2 gap-4">
            ...
            <Card title={t('monthExpense')} sub={t('block3SubExpense')} pill="Block 3">
              <div className="mt-4 text-2xl font-bold">{dash ? fmtJPY(Math.abs(dash.fbaFees) + Math.abs(dash.ads) + Math.abs(dash.refunds)) : '-'}</div>
            </Card>
          </section>
              ...
              <div className="flex items-center gap-2">
                <button
                  className={cls(
                    ...
                    tab === 'transactions' ? 'bg-[#2b5cff] text-white' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                  )}
                ...
                <button
                  className={cls(
                    'rounded-xl px-3 py-2 text-sm font-medium',
                    tab === 'import' ? 'bg-[#2b5cff] text-white' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                  )}
                    ...
                    {items.map((tx) => (
                      <tr key={tx.id} className="border-t hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-right">
                          <button className="rounded-xl border px-3 py-1.5 text-sm hover:bg-slate-50" onClick={() => deleteTx(tx.id)}>
                            {t('delete')}
                          </button>
                        </td>
                      </tr>
        ...
        <aside className="col-span-12 lg:col-span-3 space-y-4">
          ...
          <section className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 rounded-2xl border bg-[#f6f9ff] p-4">
          ...
          <section className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 rounded-2xl border bg-[#f6f9ff] p-4">
      ...
      <button
        ...
        title="Back to top"
      >
        <span className="text-[#2b5cff] text-xl leading-none">↑</span>
      </button>
    </div>
  );
}

function Card({ title, sub, pill, children }: { title: string; sub: string; pill?: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
...
function SparkCard({ title }: { title: string }) {
    ...
    <div className="rounded-2xl border bg-white p-3">
      ...
      <div className="mt-2 h-10 w-full">
        <svg viewBox="0 0 100 40" className="h-full w-full">
          <polyline points={pts} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#2b5cff]" />
        </svg>
      </div>
    </div>
  );
...
function CostRow({ label, pct, amount }: { label: string; pct: number; amount: string }) {
  return (
    <div className="mt-2 rounded-2xl border bg-white p-3">
      ...
      <div className="mt-2 h-2 w-full rounded-full bg-slate-100">
        <div className="h-2 rounded-full bg-[#2b5cff]" style={{ width: `${Math.min(100, Math.max(0, pct))}%` }} />
      </div>
...
function QuickCard({ title, sub, btn }: { title: string; sub: string; btn: string }) {
  return (
    <div className="rounded-2xl border bg-white p-4 shadow-sm">
      <div className="text-sm font-semibold text-slate-900">{title}</div>
      <div className="mt-1 text-[12px] text-slate-500">{sub}</div>
      <button className="mt-3 rounded-full bg-[#eaf1ff] px-3 py-1 text-sm font-semibold text-[#2b5cff] hover:opacity-90">
        {btn}
      </button>
    </div>
  );
...
function NoticeItem({ tag, date, text }: { tag: string; date: string; text: string }) {
  return (
    <div className="rounded-2xl border bg-white p-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">{tag}</span>
          <span className="text-[12px] text-slate-400">{date}</span>
        </div>
        <span className="h-2 w-2 rounded-full bg-[#2b5cff]" />
      </div>
