...
export default function DataImportPage() {
  ...
  const [meta, setMeta] = useState<ImportJobsMetaResponse | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      if (!jobsRes.ok) {
        throw new Error(`/api/import-jobs failed: ${jobsRes.status}`);
      }
      if (!metaRes.ok) {
        throw new Error(`/api/import-jobs/meta failed: ${metaRes.status}`);
      }

      const jobsJson = (await jobsRes.json()) as ImportJobsResponse;
      const metaJson = (await metaRes.json()) as ImportJobsMetaResponse;
    ...
    return (
      <main className="space-y-6">
        <section className="ls-card-solid rounded-[28px] p-6">
          <div className="text-sm text-slate-500">import jobs を読み込み中...</div>
        </section>
      </main>
    );
  }
    ...
    return (
      <main className="space-y-6">
        <section className="rounded-[28px] border border-rose-200 bg-rose-50 p-6">
          <div className="text-sm font-semibold text-rose-700">データインポート情報の取得に失敗しました</div>
      ...
      <section className="overflow-hidden rounded-[32px] border border-white/60 bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_55%,#334155_100%)] p-7 text-white shadow-[0_18px_40px_rgba(15,23,42,0.18)]">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div className="rounded-[22px] bg-white/92 p-5 text-slate-900 shadow-sm">
            ...
            <div className="mt-2 text-xl font-semibold">{meta?.summary.pending ?? 0}</div>
          </div>
          <div className="rounded-[22px] bg-white/92 p-5 text-slate-900 shadow-sm">
