...
const I18N: Record<Lang, T> = {
  ja: {
    ...
    ai: {
      title: "Businessプランの高度機能",
      lead: "数字を“解釈できる情報”へ。経営判断を速くします。",
      items: ["AI月次サマリー（予定）", "異常・リスク検知（予定）", "対話式検索（予定）", "証憑の自動読取・照合（予定）"],
  ...
  "zh-CN": {
    ...
    hero: { eyebrow: "价格方案", h1: "从免费开始，用得上再升级。", h2: "年付 25% 优惠。按経営規模选择最合适方案。", note: "※ 经营参考工具，最终申告以税理士判断为准。" },
    ...
    simulator: { title: "経営規模模拟器", lead: "输入月销售额，推荐合适方案（仅供参考）。", input: "月销售额（JPY）", placeholder: "例如：1500000", rec: "推荐方案", why: "推荐理由", tip: "※ 仅供参考；店铺数/SKU/广告规模会影响最优选择。" },
    ...
    ai: { title: "AI Premium 专属強化", lead: "把数字变成能用的信息。", items: ["AI月次サマリー（予定）", "異常・リスク検知（予定）", "対話式検索（予定）", "証憑识别与照合（予定）"], note: "※ AI 仅提供经营参考，不构成税务/法律意见。" },
    faq: { title: "FAQ", items: [{ q: "免费体验后会自动扣费吗？", a: "目前不会。未来上线订阅也会由你确认后才开始计费。" }, { q: "年付能省多少？", a: "年付按月换算 25%OFF（0.75 倍），节省额自动计算。" }, { q: "可以随时升级/降级吗？", a: "可以（规划）。" }] },
    bottom: { title: "先免费开始，10 分钟看到第一份利润概要。", lead: "规模变大再升级，不浪费。", cta: "现在开始免费体验", sub: "※ 最终申告以税理士判断为准（本服务仅供经营参考）。" },
  },
  ...
  "zh-TW": {
    ...
    ai: { title: "AI Premium 專屬強化", lead: "把數字變成能用的資訊。", items: ["AI 月度經營解讀", "異常與風險提示", "對話式查詢", "發票識別與照合（計畫）"], note: "※ AI 僅提供經營參考，不構成稅務/法律意見。" },
...
const PLANS: Record<Lang, Plan[]> = {
  ...
  "zh-CN": [
    ...
    { key: "ai", name: "Business", monthly: 3980, tagline: "AI 把数字变成信息", items: ["AI 月度解读（予定）", "異常/リスク提示（予定）", "対話式検索（予定）"], fit: ["月销500万以上", "多人协作/外包", "更快决策"] },
  ],
  "zh-TW": [
